...
# Home APM â€” an APM for your house

**Home Assistant has always recorded automation traces â€” it just never let
...
graph â€” the cryptic `conditions/0/conditions/1/conditions/0` node paths turn
into named, clickable spans â€” alongside sensor metrics, a room dashboard, and
...
`persistent_notification.create` â€” its `ha.template_errors` reads
...
> those two million users share one specific, documented pain â€” reading
...
> need is often already gone â€” *"Chosen trace is no longer available."* The
> demand for a fix is explicit: a separate thread asks, verbatim, whether Home
> Assistant can export its traces to an OpenTelemetry Collector â€” a change one
> user calls a **"tremendous benefit"** â€” and today there is no solution,
...
Â· [community thread 767431 ("mostly useless")](https://community.home-assistant.io/t/767431)
Â· [community thread 795531 (asks for OTel export)](https://community.home-assistant.io/t/795531)

### Before / after â€” the same automation, two tools
...
| Icon-only node graph, no per-step durations, `runtime: 0.00 seconds`, and "Not all shown activity might be related to this automation." | A named waterfall: `trigger â†’ choose â†’ choose branch 0 â†’ condition: template â†’ light.turn_on`, each span timed, with `ha.step_type`, `ha.result`, and `ha.node_path` on the side panel. |
...
flowchart LR
    subgraph HA["Home Assistant"]
      AE["automation engine<br/>(trace/get Â· state_changed)"]
    end
    subgraph SIDE["Home APM sidecar (this repo)"]
      direction TB
      WS["ws_client<br/>WS auth Â· subscribe Â· raw trace/get"]
      TR["trace_reconstruct<br/>PURE fn: payload dict â†’ SpanSpec list"]
      EM["otlp_emit<br/>mints run_id â†’ trace_id<br/>CLIENT/SERVER span.kind â†’ service map"]
      MET["metrics Â· logs_bridge Â· selfobs"]
      WS --> TR --> EM
      WS --> MET
    end
    subgraph SZ["SigNoz (self-hosted)"]
      ...
      M["metrics"]
      L["logs"]
      D["dashboards Â· alerts Â· service map"]
    end
    AE -- "WebSocket" --> WS
    EM -- "OTLP/HTTP :4318" --> T
...
it touches the network â€” which is exactly what makes it golden-testable offline:
...
```
service.name  ha.automation (root, SERVER)  Â·  ha.light / ha.climate / ha.cover /
              ha.input_boolean / ha.input_number / ha.persistent_notification
              (service-call children, CLIENT, peer.service = target domain)
span attrs    automation.name Â· automation.id Â· automation.room Â· ha.node_path Â·
              ha.step_type {trigger|condition|choose|sequence|wait|repeat|
              parallel|service_call} Â· ha.context_id Â· ha.run_id Â· ha.result Â·
              ha.changed_variables Â· ha.template_errors Â· + otel status/error
trace_id      minted by the sidecar per run; the sidecar owns the run_idâ†’trace_id map
```
...
**service map of your house** from the spans â€” see the demo tour below.

---

## Quickstart

Two modes, by design. **Demo mode** is a zero-config seeded house â€” the point is
...
### Offline first (no house, no Docker) â€” the replicability floor

```bash
pip install -e ".[dev]"
pytest          # 68 tests, all green â€” the reconstruction is verified against
...
### Demo mode â€” seeded house on SigNoz (`foundryctl` / casting)
...
### Demo / BYOH â€” plain `docker compose` (the always-works fallback)
...
For **BYOH**, copy `deploy/homeapm.env.example â†’ deploy/homeapm.env`, paste a
...
â€” tokens live in the gitignored `.ha-runtime/` directory or the environment.
...
**1 Â· Latency â€” where did my morning go?**
...
**2 Â· Parallel + repeat + error â€” is this even a real tracer?**
...
get wrong â€” real per-element start times make the parallel branches provably
correct rather than smeared into a fake sequence.

**3 Â· Logs â†” traces â€” the bridge narrates itself**
...
`converted run <ha_run_id> â†’ trace <trace_id> (N spans)`. Because the sidecar
owns the `run_id â†’ trace_id` map, that is a **100% join** between a Home
Assistant run id and its SigNoz trace id â€” every trace you see has a log that
names the exact run it came from. (This is an id-level join in the log body, not
a native clickable trace badge â€” see Limitations.)

**4 Â· The board â€” a dashboard my partner could read**
...
and â€” in red â€” the failing `ha.persistent_notification`.

**6 Â· The alert closes the loop back into Home Assistant**
...
â€” e.g. **"SigNoz [FIRING]: Automation failing"**, which even names the
...
causal fact comes from real trace data â€” the LLM only translates language at the
...
- **68 tests, all green** â€” the reconstruction algorithm is verified against
...
- **Module split** keeps the moat pure: `trace_reconstruct` is a dependency-free
  `dict â†’ [SpanSpec]` function; all I/O (`ws_client`, `otlp_emit`, `metrics`,
  `logs_bridge`, `selfobs`, `replay`) lives outside it.
- **Python 3.11**, four runtime dependencies (`websockets`, `httpx`, the two
  OpenTelemetry packages) â€” no Home Assistant install required to run the tests.
...
- **Start timestamps are real; per-step *end* is inferred.** Home Assistant's
  ...
  branches start at their true times â€” that is what makes the overlapping bars
  correct. But Home Assistant stores **no per-step end time**, so a step's
  duration is still `(next in-scope element start âˆ’ this start)`, and a
  terminal/leaf span's end bounds to the parent/trace finish. This is
  correctly-scoped inference, not zero inference â€” do not read the flame graph as
  if every bar's right edge were independently measured.
- **Logsâ†”traces is an id-join, not a native badge.** The correlation shipped is
  the sidecar narrating `run_id â†’ trace_id` in its own OTLP log body (a 100%
...
- **Tree-builder discipline.** The reconstruction is frozen and guarded by the
  ...
  core, not a late hack â€” but it is the one piece you do not touch the night
  before a demo.
...
- **No tested one-click HAOS add-on.** Supervisor add-ons only run on
  ...
  this repo ships the sidecar + casting/compose paths â€” **not** a verified
...
per the hackathon rules (mandatory â€” omission is a disqualification). Anthropic's
...
- **Author tooling**: the dashboard/alerts/views appliers, the "ask your house"
  ...
  `docs/screenshots/` (all from the live local stack â€” no image was fabricated).
...
- **`ha-kafka-net`** â€” bridges Home Assistant to .NET via Kafka for building
...
- **`detektr`** â€” a computer-vision / detection pipeline in the HA ecosystem that
...
`trace/get` node-path payloads â€” the data the built-in viewer already has but
renders unusably â€” into standards-compliant OTLP span trees on SigNoz.
